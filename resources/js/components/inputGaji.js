...
const inputGaji = () => {
    ...
    useEffect(() => {
        let dt = new Date();
        form.setFieldsValue({
            tahun: dt.getFullYear().toString(),
            bulan:dt.getMonth().toString()
        });
    }, []);
    ...
    const onFinish = values => {
        ...
        console.log(body);
        Axios.post(api.inputGaji, body, {
            headers: {
                Authorization: "Bearer " + localStorage.token
            }
        })
            .then(ress => {
                toggleNotif("success", "Berhasil menambahkan gaji");
